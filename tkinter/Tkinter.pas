...
uses
     ...
     Vcl.Samples.Spin, mxClickSplitter, dxGDIPlusClasses, Handles;
...
type
  ...
  public
     ...
     procedure PanelOnMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
  end;
...
procedure TForm_TKinter.ControlMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TForm_TKinter.ControlMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
     oControl  : TControl;
     //
     iTag      : Integer;
...
begin
     ...
     Caption   := Format('L= %d ,W=%d',[oControl.Left,oControl.Width]);
     //
     Memo1.Lines.Text    := joNode.ToJSON(False);
end;
...
procedure TForm_TKinter.Panel_FormClientMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
     oPanel    : TPanel;
     joNode    : TJsonObject;
...
procedure TForm_TKinter.PanelOnMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
     oControl  : TControl;
begin
     if Sender = Panel_FormClient then begin
...
procedure TForm_Tkinter.PanelOnResize(Sender: TObject);
begin
     //
     Caption   := Format('Resize L= %d, T= %d, W=%d, h=%d',[TPanel(Sender).Left,TPanel(Sender).Top,TPanel(Sender).Width,TPanel(Sender).Height]);
end;
...
function TForm_TKinter.CreateParentPanel(AIndex:Integer):TPanel;
begin
     ...
     Result.Tag               := -(AIndex+1);
     //
     Result.OnResize          := PanelOnResize;
     //Result.OnMouseMove       := PanelOnResize;
end;


//
function TForm_TKinter.CreateComponent(ANode: TJsonObject;AIndex:Integer): TControl;
var
     //
     oPanel         : TPanel;
     //
